...
const Privacy = () => {
  ...
  const getPrivacyPage = async () => {
    const url = `${process.env.REACT_APP_BACKEND_URL}/api/privacy`;
    const options = {
      method: "GET",
      url: url,
    };
    await Axios(options).then((res) => {
      setPrivacyPage(res.data.data[0].privacyText);
      console.log(res.data.data[0].privacyText);
    });
  };
  ...
  const handlePrivacySubmit = async (event) => {
    ...
    const privacyTextData = data.get("privacyText");
    ...
    const options = {
      ...
      data: { privacyText: privacyTextData },
    };
    await Axios(options)
      .then((res) => {
